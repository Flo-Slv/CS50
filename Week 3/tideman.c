...
bool is_cycle(int winner, int loser)
{
    ...
    for (int i = 0; i < candidate_count; i++)
    {
        // Check if winner is locked with a candidate.
        if (locked[i][winner])
        {
            return is_cycle(i, loser); // Recursive to check if that candidate is locked with loser.
        }
    }
...
void lock_pairs(void)
{
    for (int i = 0; i < pair_count; i++)
    {
        // Lock the pair if is_cycle return false.
